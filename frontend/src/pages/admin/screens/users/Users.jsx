...
const Users = () => {
    const {
        ...
        dataQueryKey: "users",
        deleteDataMessage: "Akun pengguna dihapus!",
    ...
    const { mutate: mutateUpdateUser, isLoading: isLoadingUpdateUser } = useMutation({
        ...
        onSuccess: (data) => {
            ...
            toast.success("Akun pengguna berhasil diupdate!");
        },
        onError: (error) => {
    ...
    const handleAdminCheck = (event, userId) => {
        ...
        if (window.confirm("Apakah Anda ingin mengubah status admin pengguna ini?")) {
    ...
    return (
        ...
        dataListName="Daftar Pengguna"
        searchInputPlaceHolder="Email Pengguna"
        ...
        {usersData?.data.map((user) => (
            ...
            <td className="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <input
                type="checkbox"
                className="d-checkbox disabled:bg-orange-400 disabled:opacity-100 checked:bg-[url('../../../../../public/images/check.png')] checked:text-[50px] bg-cover checked:disabled:bg-none"
