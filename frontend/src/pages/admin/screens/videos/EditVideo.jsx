...
import { getSingleVideo, updateVideo } from '../../../../services/index/videos';
...
const EditVideo = () => {
    ...
    const { mutate: mutateUpdateVideoDetail, isLoading: isLoadingUpdateVideoDetail, } = useMutation({
      ...
      onSuccess: (data) => {
        queryClient.invalidateQueries(["videdu", slug]);
        toast.success("Video berhasil diupdate!");
        navigate(`/admin/videos/manage/edit/${data.slug}`, { replace: true });
      },
      onError: (error) => {
    ...
    const handleUpdatePost = async () => {
      ...
      updatedData.append(
        "document",
        JSON.stringify({ body, categories, title, tags, slug: postSlug, vidurl, caption })
      );

      mutateUpdateVideoDetail({
        updatedData,
        slug,
    ...
    const handleDeleteImage = () => {
      if (window.confirm("Apakah Anda yakin ingin menghapus sampul video?")) {
                ...
                <article className='flex-1'>
                  ...
                  <button type='button' onClick={handleDeleteImage} className='w-fit bg-red-600 text-sm text-white font-semibold rounded-lg px-2 py-1 mt-5'>
                    ...
                    <div className="d-form-control w-full">
                      ...
                      placeholder='Link ID Video YouTube' />
                    </div>
                    ...
                    <button 
                    type='button' 
                    disabled={isLoadingUpdateVideoDetail}
